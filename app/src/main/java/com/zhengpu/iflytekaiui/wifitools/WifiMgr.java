package com.zhengpu.iflytekaiui.wifitools;
...
//				resList = sortByLevel(tmpList);
    ...
    public boolean connectWifi(final String ssid, final String pwd, List<ScanResult> scanResults) throws InterruptedException {
        ...
        final String security = WifiUtil.ConfigSec.getScanResultSecurity(result);
        final WifiConfiguration config = WifiUtil.getWifiConfiguration(mWifiManager, result, security);
        ...
        if (config == null) {
            ...
            String scanResultSecurity = WifiUtil.ConfigSec.getScanResultSecurity(result);
            boolean isOpenNetwork = WifiUtil.ConfigSec.isOpenNetwork(scanResultSecurity);
            if (isOpenNetwork) {
                connResult = WifiUtil.connectToNewNetwork(mContext, mWifiManager, result, null, numOpenNetworksKept);
            } else {
                connResult = WifiUtil.connectToNewNetwork(mContext, mWifiManager, result, pwd, numOpenNetworksKept);
            }
            return connResult;
        } else {
            ...
            if (!isCurrentNetwork_ConfigurationStatus && !isCurrentNetwork_WifiInfo) {
                //连接已保存的WiFi
                String scanResultSecurity = WifiUtil.ConfigSec.getScanResultSecurity(result);
                final WifiConfiguration wcg = WifiUtil.getWifiConfiguration(mWifiManager, result, scanResultSecurity);
                boolean connResult = false;
                if (wcg != null) {
                    connResult = WifiUtil.connectToConfiguredNetwork(mContext, mWifiManager, wcg, false);
                }
                return connResult;
            } else {
                //点击的是当前已连接的WiFi
